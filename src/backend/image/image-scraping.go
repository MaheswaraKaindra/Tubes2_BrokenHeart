...
func downloadImage() error {
	jsonPath := filepath.Join("..", "data", "images.json")
	outputDir := filepath.Join("..", "..", "frontend", "public", "data")
	...
	for _, entry := range images {
		filename := filepath.Join(outputDir, entry.Name+".svg")
		resp, err := http.Get(entry.Img)
		if err != nil {
